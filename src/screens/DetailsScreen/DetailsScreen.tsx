...
const DetailsScreen = ({ route, navigation }: any) => {
  const { parkId, name, image, description, facilities } = route.params;
  const [note, setNote] = useState(''); // Not state'i
  const [modalVisible, setModalVisible] = useState(false); // Modal görünürlüğü

  // Favorilere ekleme
  const handleAddFavorite = async () => {
    const favorite: Favorite = { id: parkId, name, image, description, facilities, note };
  ...
  const handleDownloadImage = async () => {
    try {
      const fileUri = `${FileSystem.documentDirectory}${name.replace(/\s/g, '_')}.jpg`;
